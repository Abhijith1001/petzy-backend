...
router.post("/login", async (req, res) => {
	try {
		...
		req.session.shelterID = shelter._id;
		console.log(req.session.shelterID);

		const token = shelter.generateAuthToken();
		res.status(200).send({ data: token,shelter_id: shelter._id, message: "logged in successfully" });
